...
public var middleBar:FlxSprite;
...
function create() {
    //BARS
    insert(0,rightBar = new FlxSprite().loadGraphic(Paths.image('game/healthbar/' +'/bar')));
    rightBar.color = (boyfriend.iconColor != null ? boyfriend.iconColor : 0xFF00FF00);

    insert(1,leftBar = new FlxSprite().loadGraphic(Paths.image('game/healthbar/' + '/bar')));
    ...
    //BORDER
    insert(2,hpBorder = new FlxSprite().loadGraphic(Paths.image('game/healthbar/' + '/border')));
...
function postCreate(){//here let me do it for you
    ...
    if (far){
        ...
        insert(0,middleBar = new FlxSprite().loadGraphic(Paths.image('game/healthbar/' + '/bar')));
