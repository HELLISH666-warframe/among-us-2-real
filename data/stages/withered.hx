...
override function update(elapsed:Float){if (FlxG.save.data.chrom) {time += elapsed;
	...
	chrom.data.bOffset.value = [-0.013*Math.sin(time)];}}
function postCreate() {
    ...
    fx.setGraphicSize(Std.int(2060)); // i dont know why but this gets smol if i make it the same size as the kade ver
    fx.scale.x=0.45;
...
//            FlxTween.tween(bgbleffect, {alpha: 0}, 1, {ease: FlxEase.quadInOut});
        ...
        case 1120:
            ...
            for (i in 0...playerStrums.members.length) FlxTween.tween(playerStrums.members[i], {x: playerStrums.members[i].x - 220}, 1, {ease: FlxEase.backOut});
