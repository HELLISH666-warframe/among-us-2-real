...
for (i in 31...songs.length)
	{
		grpSongs.members[i].y += (Math.sin(i+time)*2)/1;
		grpSongs.members[i].angle += (Math.sin(i+time)*9)/3;
	}
...
function shadering() 
	{
		...
		switch(curSong)
		{
			case "trojan-virus" | "file-manipulation" | "Recycle-Bin" | "file-manipulation-b" | "Bleeding":{wig.data.on.value = [1.];}
			FlxG.camera.angle = 10;
			camText.angle = 10;
			FlxG.camera.addShader(glitchShader);
			default:{wig.data.on.value = [0.];}	
			FlxG.camera.zoom = 1;
			FlxG.camera.angle = 0;
			camText.angle = 0;
			FlxG.camera.removeShader(glitchShader);			FlxG.camera.removeShader(glitchShader);			FlxG.camera.removeShader(glitchShader);			FlxG.camera.removeShader(glitchShader);
		}
		diffText.color = switch(diffText.text)
		{
			case 'ERROR':FlxColor.RED;
			case 'FLIP':FlxColor.BLUE;
